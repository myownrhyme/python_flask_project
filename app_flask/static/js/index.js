$(function () {
    ...
    function fInitialize() {
        var that = this;
        // 常用元素
        that.listEl = $('div.index-load-more');
        // 初始化数据
        that.uid = window.uid;
        that.page = 1;
        that.pageSize = 10;
        ...
        var sImageId = $('#jsCmt').closest('article').attr('id');
        ...
        var bSubmit = false;
        $('#jsSubmit').on('click',function(oEvent){
        ...
        that.requestData({
            ...
            call: function (oResult) {
                ...
                $.each(oResult.images, function (nIndex, oImage) {
                    sHtml += that.tpl(
                    ['<article class="mod"><header class="mod-hd"><time class="time">#{created_date}</time><a href="/profile/#{image_id}',
                    '" class="avatar"><img src="#{head_url}"></a><div class="profile-info"><a title="#{image_username}',
                    '" href="/profile/#{user_id}">#{image_username}',
                    '</a></div></header><div class="mod-bd"><div class="img-box"><a href="/image/#{image_id}"><img src="#{url}"></a>',
                    '</div></div><div class="mod-ft"><ul class="discuss-list"><li class="more-discuss"><a><span>全部 </span><span class="">#{comment_count}',
                    '</span><span> 条评论</span></a></li>'].join(''),oImage);
                    var ssHtml = '';
                    $.each(oImage.comment,function(nIndex,oComment){
                        ssHtml += that.tpl(
                        ['<li><a class="_4zhc5 _iqaka" title="',
                         '#{username}" href="/profile/#{user_id}',
                         '" data-reactid=".0.1.0.0.0.2.1.2:$comment-17856951190001917.1">#{username}',
                         '</a><span><span>#{content}</span></span></li>'].join(''),oComment);
                    })
                    sHtml+=ssHtml;
                    sHtml+=that.tpl([
                    '</ul><section class="discuss-edit"><a class="icon-heart"></a><form><input placeholder="添加评论..." type="text">',
                    '</form><button class="more-info">更多选项</button></section></div></article>',
                       ].join(''),oImage);
                });
